...
export default function EmployeesPage() {
  ...
  return <section className="space-y-6">
    ...
    {canRegister && showForm && <form className="space-y-3 rounded-xl border bg-background p-4" onSubmit={async (event) => {
      ...
      <fieldset className="space-y-2 rounded-md border p-3 sm:col-span-2 lg:col-span-3"><legend className="px-1 text-sm">Branch assignments *</legend><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{branches.map((item) => <label key={item.id} className="flex items-center gap-2 text-sm"><input type="checkbox" checked={selectedBranchIds.includes(item.id)} onChange={() => setSelectedBranchIds((current) => current.includes(item.id) ? current.filter((id) => id !== item.id) : [...current, item.id])}/>{item.name}</label>)}</div></fieldset>
