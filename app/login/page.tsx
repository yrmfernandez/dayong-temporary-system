  ...
  return <main className="grid min-h-dvh overflow-y-auto bg-white lg:h-dvh lg:grid-cols-[1.08fr_0.92fr] lg:overflow-hidden">
    <section className="relative hidden overflow-y-auto bg-violet-10 px-10 py-9 text-white lg:flex lg:flex-col xl:px-16">
      ...
      <div className="relative z-10 my-auto max-w-2xl py-10"><div className="mb-7 flex size-12 items-center justify-center rounded-2xl bg-accent-lime text-violet-10 shadow-lg"><HeartHandshake className="size-6"/></div><h1 className="max-w-xl text-4xl font-black leading-tight tracking-tight xl:text-5xl">Serving and protecting the needs of every member.</h1>
        ...
        <details className="group mt-4 rounded-2xl border border-white/15 bg-white/10 p-5 backdrop-blur"><summary className="cursor-pointer list-none text-sm font-bold text-accent-lime">Dayong Workplace Prayer <span className="float-right text-violet-80 group-open:rotate-180">&darr;</span></summary><p className="mt-4 whitespace-pre-line text-xs leading-5 text-violet-95">{prayer}</p></details>
      </div>
      <div className="relative z-10 flex items-center justify-between border-t border-white/10 pt-4 text-xs text-violet-80"><span>&copy; 2026 Dayong Monitoring System</span><a href="https://www.facebook.com/dsrdprovidersinc" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 font-semibold hover:text-white">Facebook Page <ExternalLink className="size-3"/></a></div>
    </section>
    <section className="relative flex min-h-dvh flex-col overflow-hidden bg-gradient-to-br from-white via-violet-95/45 to-purple-95/50 px-5 py-5 sm:px-12 sm:py-6 lg:px-16 xl:px-24"><div className="flex items-center gap-3 lg:hidden"><BrandLogo className="size-14 rounded-full object-contain" priority/><div><p className="font-black text-violet-10">DAYONG</p><p className="text-[10px] uppercase tracking-wider text-violet-40">Monitoring System</p></div></div>
      <div className="relative my-auto w-full max-w-md self-center rounded-3xl border border-violet-90/80 bg-white/90 p-6 shadow-[0_24px_70px_-28px_rgb(45_28_89_/_0.45)] backdrop-blur sm:p-9"><div className="mb-8"><span className="rounded-md bg-purple-95 px-2 py-1 text-[11px] font-bold uppercase tracking-wider text-purple-60">Secure Employee Access</span><h2 className="mt-3 text-3xl font-black tracking-tight text-violet-10 sm:text-4xl">Welcome back</h2><p className="mt-2 text-sm text-violet-40">Enter your employee account credentials to continue.</p></div>
      ...
      </div><p className="text-center text-xs text-muted-foreground">Authorized personnel only &middot; Credentials are protected by a secure session.</p></section>
  </main>;
}
