  ...
  return <main className="grid h-screen overflow-y-auto bg-white lg:grid-cols-[1.08fr_0.92fr] lg:overflow-hidden">
    <section className="relative hidden overflow-y-auto bg-violet-10 px-10 py-9 text-white lg:flex lg:flex-col xl:px-16">
      ...
      <div className="relative z-10 my-auto max-w-2xl py-10"><div className="mb-7 flex size-12 items-center justify-center rounded-2xl bg-accent-lime text-violet-10 shadow-lg"><HeartHandshake className="size-6"/></div><h1 className="max-w-xl text-4xl font-black leading-tight tracking-tight xl:text-5xl">Serving and protecting the needs of every member.</h1>
        ...
        <details className="group mt-4 rounded-2xl border border-white/15 bg-white/10 p-5 backdrop-blur"><summary className="cursor-pointer list-none text-sm font-bold text-accent-lime">Dayong Workplace Prayer <span className="float-right text-violet-80 group-open:rotate-180">âŒ„</span></summary><p className="mt-4 whitespace-pre-line text-xs leading-5 text-violet-95">{prayer}</p></details>
      </div>
      <div className="relative z-10 flex items-center justify-between border-t border-white/10 pt-4 text-xs text-violet-80"><span>Â© 2026 Dayong Monitoring System</span><a href="https://www.facebook.com/dsrdprovidersinc" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 font-semibold hover:text-white">Facebook Page <ExternalLink className="size-3"/></a></div>
    </section>
    <section className="flex min-h-dvh flex-col px-5 py-5 sm:px-12 sm:py-6 lg:px-16 xl:px-24"><div className="flex items-center gap-3 lg:hidden"><BrandLogo className="size-14 rounded-full object-contain" priority/><div><p className="font-black text-violet-10">DAYONG</p><p className="text-[10px] uppercase tracking-wider text-violet-40">Monitoring System</p></div></div>
      ...
      </div><p className="text-center text-xs text-muted-foreground">Authorized personnel only Â· Credentials are protected by a secure session.</p></section>
  </main>;
}
